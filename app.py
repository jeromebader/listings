...
### Protection Admin views, Adminrechte mit roleid =1
...
### Ist user überhaupt aktiviert oder deactiviert?
### Protection Admin views, Adminrechte mit roleid =1
def active_user(f):
    @wraps(f)
    def wrap(*args, **kwargs):
...
## Sign-up form Without Mysql query
...
## Response from pages, Gibt Rückmeldung von Klicks oder der Seiten
...
## LANDING PAGE
@app.route('/testus1', methods=['GET', 'POST'])
@login_required
@active_user
def testus1(**args):
    return render_template('testus.html')
...
## Inition of the app, CHECK THREADS AND before deployment deactivate debug mode
...
## Old Sign-up model without sqlAlchemy
